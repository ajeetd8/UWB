...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
		...
		// If no error occurreds
		if(myTcb != null) {
			// Get my Thread ID
			int myThreadID = myTcb.getTid();
			return waitQueue.enqueueAndSleep(myThreadID);
		}
		...
		myTcb = scheduler.getMyTcb();
		if(myTcb != null) {
			int myProcessId = myTcb.getPid(); 		// get my parent ID
			int myThreadId = myTcb.getTid(); 	// get my ID
			if (myProcessId != -1) {
				// wake up thread waiting from parents
				waitQueue.dequeueAndWakeup( myProcessId, myThreadId );
