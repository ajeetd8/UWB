...
class TestThread2d extends Thread {
  ...
  public TestThread2d(String args[]) {
    ...
    SysLib.cout(String.format("%05d: Thread[%s] submitted\n", submissionTime % 100000, name));
  }
  ...
  public void run() {
    ...
    for (int burst = cpuBurst; burst > 0; burst -= TIMEQUANTUM) {
      ...
      SysLib.cout(String.format("%05d: Thread[%s] is running (%5d, %5d)\n", currentTime % 100000, name,
          totalExecutionTime, totalWaitTime));
    ...
    SysLib.cout(String.format("%05d: Thread[%s]: response: %5d; wait: %5d; execution: %5d; turnaround: %5d\n",
        completionTime % 100000, name, responseTime, totalWaitTime, totalExecutionTime, turnaroundTime));
    SysLib.exit();
  }
}
