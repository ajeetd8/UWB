...
public class Inode {
    ...
    /**
     * The default constructor for INode. Only one INode exist.
     */
    Inode() {
        length = 0;
        count = 0;
        ...
        if (directNumber < directSize) { // target is in direct pointers
            ...
            if (indirect < 0) // indirect is null
                ...
                if (SysLib.bytes2short(indexBlock, indirectNumber * 2) > 0) {
                    SysLib.cerr("indexBlock, indirectNumber = " + indirectNumber + " contents = "
                            + SysLib.bytes2short(indexBlock, indirectNumber * 2) + "\n");
                    return ErrorBlockRegistered;
                }
                SysLib.short2bytes(targetBlockNumber, indexBlock, indirectNumber * 2);
